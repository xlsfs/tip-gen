...
export class FontMgr {
    ...
    public static getIns ():FontMgr {
        if (!FontMgr._instance) {
            FontMgr._instance = new FontMgr();
        }
    ...
    result: {font:string, en:string, cn:string}[] = null;
    getFonts():{font:string, en:string, cn:string}[] {
    ...
    init() {
        ...
        for (var index = 0; index < lenTables; index++) {
            ...
            var arrDataFont = this.dataFont[typeFont];
            if (arrDataFont && arrDataFont.length) {
                ...
                // 复选框事件
                eleTables[index].onclick = function (event: any) {
